...
public class WeightedQuickUnionUF {
    ...
    public void union(int p, int q) {
        ...
        if (sz[rootP] < sz[rootQ]) { parent[rootP] = rootQ; sz[rootQ] += sz[rootP]; }
        else                       { parent[rootQ] = rootP; sz[rootP] += sz[rootQ]; }
        count--;
    }
    ...
    public static void main(String[] args) {
        ...
        StdOut.println("Here is the parent[] array:");
        String output = "";
            for (int i = 0; i < uf.parent.length; i++) {
                StdOut.println(uf.parent[i]);
                output += String.format(" %s", uf.parent[i]);
            }
